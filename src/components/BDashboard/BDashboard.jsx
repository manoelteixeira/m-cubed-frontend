...
const BDashboard = () => {
  ...
  const getRequestStatus = (request) => {
    const requestProposals = offers.filter(offer => offer.request_id === request.id);
              ...
              requests.map((request) => {
                ...
                return (
                  <TableRow
                    ...
                    sx={{
                      cursor: "pointer",
                      backgroundColor: status === "Active" ? "#e0f7e0" : 
                                       status.startsWith("Funded") ? "#e0e0ff" : "#ffffff",
                    }}
                  >
                    <TableCell>{request.title}</TableCell>
                    <TableCell>{request.description}</TableCell>
                    <TableCell>{parseFloat(request.value).toLocaleString("en-US", {
                          style: "currency",
                          currency: "USD",
