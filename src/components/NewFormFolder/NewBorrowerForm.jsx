  ...
  const createNewBorrower = async (newborrower) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "User and token set successfully:",
          results.borrower,
          results.token
        );
        navigate("/borrower");
      } else {
        console.error("Registration failed:", results.error); // Log detailed error
        alert(results.error || "An error occurred during registration.");
      }
    } catch (error) {
      console.error("Error creating borrower:", error);
    }
