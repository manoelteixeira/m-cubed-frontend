...
//   // Toggle row expansion and pre-fill the loan amount
//   const toggleRowExpansion = (rowId, borrowerId, loanAmount) => {
//     setExpandedRowId(expandedRowId === rowId ? null : rowId);
//     if (borrowerId && expandedRowId !== rowId) {
//       fetchBorrowerDetails(borrowerId);
//       // Pre-fill the loan amount when the row is expanded
//       setLenderProposal((prev) => ({
//         ...prev,
...
export default function LoansMarketplace({ user, token, loadLoanProposals }) {
  ...
  const fetchBorrowerDetails = async (borrowerId) => {
    try {
      ...
      setBorrowerDetails({
        ...borrower,
        credit_reports, // Optional: store credit reports if needed
  ...
  return (
    <Grid item xs={12}>
      <Paper elevation={3} sx={{ padding: 3, backgroundColor: "#f6f7f8" }}>
        ...
        {/* Table to display loan listings */}
        <TableContainer>
          <Table>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {loanListings.map((loan) => (
                <React.Fragment key={loan.id}>
                  ...
                  {/* Expandable row for borrower details and proposal form */}
                  {expandedRowId === loan.id && (
                    <TableRow key={`${loan.id}-collapse`}>
                      <TableCell colSpan={4}>
                        <Collapse in={expandedRowId === loan.id}>
                                  ...
                                  <ul>
                                    <li>
                                      <Link
                                        ...
                                        sx={{ color: "#00a250" }}
                                      >
                                        FICO Score - Verified with Experian
                                      </Link>
                                    </li>
                                    <li>
                                      <Link
