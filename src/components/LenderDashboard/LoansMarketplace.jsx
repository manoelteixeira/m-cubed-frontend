...
export default function LoansMarketplace({ user, token, loadLoanProposals }) {
  ...
  // State for favorite and hidden rows
  const [favoriteRows, setFavoriteRows] = useState([]);
  const [hiddenRows, setHiddenRows] = useState(() => {
    const storedHiddenRows = localStorage.getItem("hiddenRowsMarketplace");
    return storedHiddenRows ? JSON.parse(storedHiddenRows) : [];
  });
  ...
  // Toggle favorite row functionality
  ...
  const hideRow = (rowId) => {
    ...
    localStorage.setItem(
      "hiddenRowsMarketplace",
      JSON.stringify(updatedHiddenRows)
    );
  };
  ...
  useEffect(() => {
    loadLoanListings();
  }, [loanListingsOffset, sortByLoanListings, sortOrderLoanListings]);
  ...
  return (
    <Grid item xs={12}>
      ...
      <Box sx={{ padding: 3, backgroundColor: "#f6f7f8" }}>
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {sortedLoanListings
                .filter((loan) => !hiddenRows.includes(loan.id))
                .map((loan) => (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <IconButton
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleFavorite(loan.id);
                          }}
                          color={
                            favoriteRows.includes(loan.id) ? "error" : "default"
                          }
                        >
                          {favoriteRows.includes(loan.id) ? (
                            <FavoriteIcon sx={{ color: "#00a250" }} />
                          ) : (
                            <FavoriteBorderIcon sx={{ color: "#00a250" }} />
                          )}
                        </IconButton>
                        <IconButton
                          onClick={(e) => {
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} fullWidth>
        <DialogTitle sx={{ backgroundColor: "#00a250", color: "#fff" }}>
          Confirm Proposal Submission
        </DialogTitle>
        <DialogContent sx={{ backgroundColor: "#f6f7f8" }}>
